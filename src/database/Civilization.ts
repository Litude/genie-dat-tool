...
import {
  ...
  createReferenceIdFromString as createReferenceIdFromString,
...
const CivilizationJsonMapping: JsonFieldMapping<
  Civilization,
  CivilizationJson
>[] = [
  ...
  { field: "internalName" },
  {
    jsonField: "bonusEffectId",
    versionFrom: "1.4.0",
    toJson: (obj) =>
      createReferenceString(
        "StateEffect",
        obj.bonusEffect?.referenceId,
        obj.bonusEffectId,
      ),
  },
  {
    objectField: "bonusEffectId",
    versionFrom: "1.4.0",
    fromJson: (json, obj, loadingContext) =>
      json.bonusEffectId !== undefined
  ...
  {
    objectField: "attributes",
    fromJson: (json, obj) => {
      const result: Float32[] = Array(json.attributes.totalCount).fill(
        asFloat32(0),
      );
      Object.entries(json.attributes.entries).forEach(
        ([resourceIdString, resourceAmount]) => {
          const resourceId = +resourceIdString;
          if (resourceId >= json.attributes.totalCount) {
            throw new Error(
              `Civilization ${obj.referenceId} has ${json.attributes.totalCount} attributes, but attempted to set attribute ${resourceId}`,
            );
          } else {
...
export class Civilization {
  ...
  appendToTextFile(
    ...
    const civAttributes = this.attributes
      .map((attributeAmount, id) => {
        const attribute = attributes[id]
          ? attributes[id]
          : createDefaultAttribute(id);
        return {
          ...attribute,
    ...
    textFileWriter.indent(2).integer(this.architectureStyle).eol();
  }
...
export function writeCivilizationsToWorldTextFile(
  ...
  const textFileWriter = new TextFileWriter(
    path.join(outputDirectory, TextFileNames.Civilizations),
  );
...
export function writeCivilizationsToJsonFiles(
  outputDirectory: string,
...
export function readCivilizationsFromJsonFiles(
  inputDirectory: string,
  civilizationIds: (string | null)[],
  loadingContext: JsonLoadingContext,
) {
  ...
  civilizationIds.forEach((civilizationReferenceId, civilizationNumberId) => {
    if (civilizationReferenceId === null) {
      ...
      const civilizationJson = CivilizationSchema.parse(
        JSON5.parse(
          readFileSync(
            path.join(
              civilizationsDirectory,
              `${civilizationReferenceId}.json`,
            ),
          ).toString("utf8"),
        ),
      );
      const civilization = new Civilization();
      civilization.readFromJsonFile(
        civilizationJson,
        asInt16(civilizationNumberId),
        civilizationReferenceId,
        loadingContext,
      );
