...
export const TerrainDataJsonMapping: JsonFieldMapping<
  TerrainData,
  TerrainDataJson
>[] = [
  {
...
export const HabitatJsonMapping: JsonFieldMapping<Habitat, HabitatJson>[] = [
  ...
  },
  {
    ...
    fromJson: (json, obj, loadingContext) => {
      const terrainData: TerrainData[] = Array.from({
        length: loadingContext.terrainCount,
      }).map((_entry, index) => ({
        terrainId: asInt16<TerrainId<Int16>>(index),
        terrain: null,
        multiplier: asFloat32(0),
      }));
      const terrains = json.terrainData.reduce(
        (acc, cur) => ({
          ...acc,
          [getIdFromReferenceString(
            "Terrain",
...
export class Habitat {
  ...
  linkTerrains(terrains: (Terrain | null)[], loadingContext: LoadingContext) {
    for (let i = 0; i < this.terrainData.length; ++i) {
      this.terrainData[i].terrain = getDataEntry(
        terrains,
        this.terrainData[i].terrainId,
        "Terrain",
        this.referenceId,
        loadingContext,
      );
  ...
  appendToTextFile(file: TextFileWriter) {
    ...
    const sortedTerrainData = [...this.terrainData].sort((a, b) => {
      if (a.terrain && b.terrain) {
        ...
        throw new Error(
          `Tried to write Habitats but terrain was null! Have they been linked?`,
        );
      }
    });

    const validCount = sortedTerrainData.filter(
      (terrainData) => terrainData.multiplier,
    ).length;
...
export function readHabitatsFromJsonFiles(
  ...
  habitatIds.forEach((habitatId, index) => {
    if (habitatId !== null) {
      const habitatJson = HabitatSchema.parse(
        JSON5.parse(
          readFileSync(
            path.join(habitatDirectory, `${habitatId}.json`),
          ).toString("utf8"),
        ),
      );
      const habitat = new Habitat();
      habitat.readFromJsonFile(
        habitatJson,
        asInt16(index),
        habitatId,
        loadingContext,
...
export function writeHabitatsToWorldTextFile(
  ...
  const textFileWriter = new TextFileWriter(
    path.join(outputDirectory, TextFileNames.Habitats),
  );
...
export function writeHabitatsToJsonFiles(
  outputDirectory: string,
  habitats: Nullable<Habitat>[],
