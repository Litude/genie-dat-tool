...
export const SoundSampleJsonMapping: JsonFieldMapping<
  SoundSample,
  SoundSampleJson
>[] = [
...
export const SoundEffectJsonMapping: JsonFieldMapping<
  SoundEffect,
  SoundEffectJson
>[] = [
  { field: "internalName" },
  { field: "playDelay", flags: { internalField: true } },
  {
    ...
    toJson: (obj, savingContext) =>
      obj.samples.map((sample) =>
        transformObjectToJson(sample, SoundSampleJsonMapping, savingContext),
      ),
  },
  {
    objectField: "samples",
    fromJson: (json, _obj, loadingContext) =>
      json.samples.map((sample) =>
        transformJsonToObject(sample, SoundSampleJsonMapping, loadingContext),
      ),
  },
...
export class SoundEffect {
  ...
  readFromBuffer(
    ...
    if (this.id !== id) {
      onParsingError(
        `Mismatch between stored Sound Effect id ${this.id} and ordering ${id}, data might be corrupt!`,
        loadingContext,
      );
    }
    ...
    for (let i = 0; i < sampleCount; ++i) {
      this.samples.push({
        ...
        resourceId: semver.gte(loadingContext.version.numbering, "1.3.1")
  ...
  readFromJsonFile(
    ...
    if (this.samples.length > 0) {
      const sampleSum = this.samples.reduce(
        (acc, cur) => acc + cur.playbackProbability,
        0,
      );
      if (sampleSum > 100) {
        Logger.warn(
          `Sound effect ${referenceId} sum of sample playback probabilities is ${sampleSum}, max expected 100`,
        );
      }
    }
  }
  ...
  appendToTextFile(
    ...
    textFileWriter.integer(this.id).integer(this.samples.length).eol();
      ...
      textFileWriter
        ...
        .integer(
          semver.gte(savingContext.version.numbering, "1.3.1")
            ? sample.resourceId
            : asTribeResourceId(sample.resourceId),
        )
...
export function readSoundEffectsFromJsonFiles(
  inputDirectory: string,
  soundEffectIds: (string | null)[],
  loadingContext: JsonLoadingContext,
) {
  ...
  soundEffectIds.forEach((soundEffectId, index) => {
    if (soundEffectId === null) {
      throw new Error(
        `Received null sound effect entry (index ${index}) while parsing JSON but such entries are not supported!`,
      );
    }
    const soundEffectJson = SoundEffectSchema.parse(
      JSON5.parse(
        readFileSync(
          path.join(soundEffectsDirectory, `${soundEffectId}.json`),
        ).toString("utf8"),
      ),
    );
    const soundEffect = new SoundEffect();
    soundEffect.readFromJsonFile(
      soundEffectJson,
      asInt16(index),
      soundEffectId,
      loadingContext,
...
export function writeSoundEffectsToWorldTextFile(
  outputDirectory: string,
  soundEffects: SoundEffect[],
  savingContext: SavingContext,
) {
  const textFileWriter = new TextFileWriter(
    path.join(outputDirectory, TextFileNames.SoundEffects),
  );
...
export function writeSoundEffectsToJsonFiles(
  outputDirectory: string,
  soundEffects: SoundEffect[],
  savingContext: SavingContext,
) {
