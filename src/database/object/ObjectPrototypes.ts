...
import {
  ...
  AnimatedObjectPrototypeSchema,
} from "./AnimatedObjectPrototype";
import {
  BuildingObjectPrototype,
  BuildingObjectPrototypeSchema,
} from "./BuildingObjectPrototype";
import {
...
import {
  MobileObjectPrototype,
  MobileObjectPrototypeSchema,
} from "./MobileObjectPrototype";
import { ObjectType, ObjectTypes } from "./ObjectType";
import {
  ProjectileObjectPrototype,
  ProjectileObjectPrototypeSchema,
} from "./ProjectileObjectPrototype";
import {
    ...
    if (validObjects[i]) {
      ...
      if (!object) {
        throw new ParsingError(
          `Received unknown object type ${objectType} when reading units!`,
        );
      } else {
...
export function writeObjectPrototypesToWorldTextFile(
  outputDirectory: string,
  civilizations: Civilization[],
  prototypes: (SceneryObjectPrototype | null)[][],
  savingContext: SavingContext,
) {
  const textFileWriter = new TextFileWriter(
    path.join(outputDirectory, TextFileNames.ObjectPrototypes),
  );
  textFileWriter.raw(civilizations.length).eol(); // civilization count
  const civilizationObjects = civilizations
    .map((civilization, index) => ({
      civilization,
      objects: prototypes[index]
        .filter(isDefined)
        .sort((a, b) => textFileStringCompare(a.internalName, b.internalName)),
      totalObjectCount: asInt32(prototypes[index].length),
    }))
    .sort((a, b) =>
      textFileStringCompare(
        a.civilization.internalName,
        b.civilization.internalName,
      ),
    );
...
export function createBaselineObjectPrototypes(
  objectPrototypes: Nullable<SceneryObjectPrototype>[][],
): Nullable<SceneryObjectPrototype>[] {
  ...
  for (let objectIndex = 0; objectIndex < objectCount; ++objectIndex) {
    ...
    for (
      let civilizationIndex = 0;
      civilizationIndex < civilizationCount;
      civilizationIndex++
    ) {
    ...
    if (Object.keys(fieldValues).length) {
      ...
      for (const key in fieldValues) {
        mostCommonFields[key as keyof SceneryObjectPrototype] =
          mergeCommonFields(fieldValues[key]);
      }

      const baseLineObject = Object.assign(
        Object.create(Object.getPrototypeOf(objectInstance)),
        Object.fromEntries(
          Object.entries(mostCommonFields).map(([key, value]) => [
            key,
            reconstructInstances(value),
          ]),
        ),
      );
...
export function writeObjectPrototypesToJsonFiles(
  outputDirectory: string,
  baselineObjects: Nullable<BaseObjectPrototype>[],
  objectPrototypes: Nullable<BaseObjectPrototype>[][],
  savingContext: SavingContext,
) {
  const objectsDirectory = path.join(outputDirectory, "objects");

  const jsonBaselineObjects: Nullable<Partial<BaseObjectPrototypeJson>>[] =
    baselineObjects.map((object) =>
      object ? object.toJson(savingContext) : null,
    );
  ...
  const jsonObjects: Nullable<Partial<BaseObjectPrototypeJson>>[][] =
    objectPrototypes.map((civObjects) =>
      civObjects.map((object) =>
        object ? object.toJson(savingContext) : null,
      ),
    );
  ...
  for (let i = 0; i < jsonBaselineObjects.length; ++i) {
    ...
    if (baselineObject) {
      forEachObjectEntry(baselineObject, (key, value) => {
        if (value !== undefined) {
          // Only fields that we have "deleted" should be undefined, otherwise null is used. Fields that don't exist in the baseline should be kept
          jsonObjects.forEach((civObjects) => {
            const civObject = civObjects[i];
  ...
  const mergedJsonObjects = jsonBaselineObjects.map(
    (baselineObject, objIndex) =>
      baselineObject
        ? {
            baseline: {
              enabled: baselineEnableStates[objIndex],
              ...baselineObject,
            },
              ...
              (acc, curCiv, civIndex) => {
                ...
                if (curCiv[objIndex] && Object.keys(curCiv[objIndex]).length) {
                  acc[civIndex] = enabledStateDiffers
                    ? {
                        enabled: Boolean(curCiv[objIndex]),
                        ...curCiv[objIndex],
                      }
                    : curCiv[objIndex];
  ...
  mergedJsonObjects.forEach((jsonObject, index) => {
    if (jsonObject) {
      writeFileSync(
        path.join(
          objectsDirectory,
          `${baselineObjects[index]?.referenceId}.json`,
        ),
...
const MergedObjectSchema = z.object({
  baseline: SceneryObjectPrototypeSchema.merge(
    z.object({
      enabled: z.boolean(),
    }),
  )
  ...
  overrides: z.record(
    z.string().regex(/^\d+$/),
    SceneryObjectPrototypeSchema.merge(
      z.object({
        enabled: z.boolean(),
      }),
    )
...
export function readObjectPrototypesFromJsonFiles(
  inputDirectory: string,
  prototypeIds: (string | null)[],
  civilizationCount: number,
  loadingContext: JsonLoadingContext,
) {
  const objectsDirectory = path.join(inputDirectory, "objects");
  const civilizationObjects: Nullable<BaseObjectPrototype>[][] = Array.from(
    { length: civilizationCount },
    () => [],
  );
  prototypeIds.forEach(
    (objectPrototypeReferenceId, objectPrototypeNumberId) => {
      if (objectPrototypeReferenceId === null) {
        for (let i = 0; i < civilizationCount; ++i) {
          civilizationObjects[i].push(null);
        }
      } else {
        const rawObject = MergedObjectSchema.parse(
          JSON5.parse(
            readFileSync(
              path.join(objectsDirectory, `${objectPrototypeReferenceId}.json`),
            ).toString("utf8"),
          ),
        );
        for (let i = 0; i < civilizationCount; ++i) {
          const fullCivilizationObject = Object.assign(
            {},
            rawObject.baseline,
            rawObject.overrides[i] ?? {},
          );
          ...
          if (enabled) {
            ...
            switch (objectType) {
              case ObjectTypes.Scenery: {
                const objectJson =
                  SceneryObjectPrototypeSchema.parse(civilizationObject);
                const object = new SceneryObjectPrototype();
                object.readFromJsonFile(
                  objectJson,
                  asInt16<PrototypeId<Int16>>(objectPrototypeNumberId),
                  objectPrototypeReferenceId,
                  loadingContext,
                );
              ...
              case ObjectTypes.Animated: {
                const objectJson =
                  AnimatedObjectPrototypeSchema.parse(civilizationObject);
              ...
              case ObjectTypes.Mobile: {
                const objectJson =
                  MobileObjectPrototypeSchema.parse(civilizationObject);
              ...
              case ObjectTypes.Actor: {
                const objectJson =
                  ActorObjectPrototypeSchema.parse(civilizationObject);
              ...
              case ObjectTypes.Combatant: {
                const objectJson =
                  CombatantObjectPrototypeSchema.parse(civilizationObject);
              ...
              case ObjectTypes.Projectile: {
                const objectJson =
                  ProjectileObjectPrototypeSchema.parse(civilizationObject);
              ...
              case ObjectTypes.AdvancedCombatant: {
                const objectJson =
                  AdvancedCombatantObjectPrototypeSchema.parse(
                    civilizationObject,
                  );
              ...
              case ObjectTypes.Building: {
                const objectJson =
                  BuildingObjectPrototypeSchema.parse(civilizationObject);
              ...
              case undefined:
                throw new Error(
                  `Object ${objectPrototypeReferenceId} for civilization ${i} has no objectType!`,
                );
              default:
                throw new Error(
                  `Unrecognized object type ${objectType} for ${objectPrototypeReferenceId} (civilization ${i})`,
                );
            }
          } else {
            civilizationObjects[i].push(null);
