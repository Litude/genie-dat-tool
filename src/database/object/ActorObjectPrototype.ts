...
export const ActorObjectPrototypeSchema = MobileObjectPrototypeSchema.merge(
  z.object({
...
const ActorObjectPrototypeJsonMapping: JsonFieldMapping<
  ActorObjectPrototype,
  ActorObjectPrototypeJson
>[] = [
  ...
  },
  {
    ...
    fromJson: (json, obj, loadingContext) =>
      json.dropSitePrototypeIds.map((prototypeId) =>
  ...
  {
    jsonField: "attackSoundId",
    toJson: (obj) =>
      createReferenceString(
        "SoundEffect",
        obj.attackSound?.referenceId,
        obj.attackSoundId,
      ),
  },
  {
    objectField: "attackSoundId",
    fromJson: (json, obj, loadingContext) =>
  ...
  {
    jsonField: "moveSoundId",
    toJson: (obj) =>
      createReferenceString(
        "SoundEffect",
        obj.moveSound?.referenceId,
        obj.moveSoundId,
      ),
  },
  {
    objectField: "moveSoundId",
    fromJson: (json, obj, loadingContext) =>
      json.moveSoundId !== undefined
        ? getIdFromReferenceString<SoundEffectId<Int16>>(
            "SoundEffect",
            obj.referenceId,
            json.moveSoundId,
  ...
  {
    jsonField: "abilityList",
    toJson: (obj, savingContext) =>
      obj.abilityList.map((ability) =>
        transformObjectToJson(ability, AbilityJsonMapping, savingContext),
      ),
  },
  {
...
export class ActorObjectPrototype extends MobileObjectPrototype {
  ...
  readFromBuffer(
    ...
    const dropSiteCount = semver.gte(loadingContext.version.numbering, "1.3.1")
      ? 2
      : 1;
  ...
  readFromJsonFile(
    jsonFile: ActorObjectPrototypeJson,
    ...
    applyJsonFieldsToObject(
      jsonFile,
      this,
      ActorObjectPrototypeJsonMapping,
      loadingContext,
    );
  ...
  linkOtherData(
    ...
    this.dropSitePrototypes = this.dropSitePrototypeIds.map((prototypeId) =>
      getDataEntry(
        objects,
        prototypeId,
        "ObjectPrototype",
  ...
  appendToTextFile(
    ...
    textFileWriter.indent(4).raw(this.abilityList.length).eol();
